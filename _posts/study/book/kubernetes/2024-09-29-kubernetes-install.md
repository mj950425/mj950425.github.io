...
다시, 네트워크와 호스트 이름을 클릭한다. 그리고 이더넷(enp0s1)에 대한 설정을 클릭한다.

그리고 수동을 클릭 IPv4 설정을 클릭하고 add를 눌러서 아래를 입력해준다.
...
만약에 리눅스를 재설치했다면, 아래 명령어를 통해서 키를 한번 지워주고 다시 실행한다.
...
만약에 yum이 설치가 안된다면 dns가 설정되어있는지 확인한다.
...
containerd가 사용하는 cgroup을 systemd로 관리하게 설정한다.
...
만약에 아래와 같은 경고 나온다면, /etc/hosts에 127.0.0.1 k8s-master 를 추가해주면 된다.
