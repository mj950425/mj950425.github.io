...
아무래도 정답이 있는 문제가 아니므로 쉽사리 하나로 결정하기 어렵기 떄문이었던것 같습니다.

아래는 제가 생각한 의견을 전달드리는 과정 중 일부를 축약해서 적어봤는데요.

다시 생각해보면 CQRS에만 너무 집중해서 의견을 말했나하는 아쉬움도 듭니다. 

마인드맵과 같은 시각자료를 준비했으면 조금 더 좋았을 것 같다는 생각도 드네요.  

- 질문 : 현재 도메인에서 CQRS시스템으로 구축할 필요가 있을까?
- 대답 : 백오피스와 여행자 서비스로 나누는것이 CQRS아키텍처를 구현한다를 의미하는게 아닙니다. 바운디드 컨택스트에 맞게 시스템을 분리함으로써 단일 도메인으로 전부 표현하려는 오류를 범하지않고 백오피스 서버가 다운되었을때 여행자 서버가 다운되지않도록 하는 기능이 가장 큽니다. 물론 이렇게 가져갔을때 CQRS의 장점도 어느정도 챙길 수 있다고 생각했구요.

- 질문 : CQRS아키텍처에서는 언젠가 데이터소스를 분리해야하지않나?
- 대답 : CQRS의 오해라고 생각하는데요. 성능 튜닝을 위해서 데이터베이스를 분리한뒤에 이벤트소싱을 통한 데이터 싱크를 맞추는 작업은 CQRS의 모든 장점이 아니라고 생각합니다. 비지니스가 다른 커멘드와 쿼리 서비스를 코드가 뒤섞이지않도록 물리적으로 분리하는것만으로도 CQRS의 장점은 충분하다고 생각합니다.

- 질문 : 재고의 경우 여행자 서버에서도 커맨드 작업이 필요한데 이 부분은 어떠한지?
- 대답 : 이러한 이유로 물론 재고 시스템도 같이 분리할 수 있으면 좋을것 같지만 너무 잘게 자르는것은 개발 생산성 저하를 가져올 수 있으므로 크게 잘랐을때 먼저 어떻게 자를것인가에 대한 고민이 있었는데요. 처음에는 크게 여행자와 백오피스로 나누고, 추후에 재고 도메인만 따로 빼내는것은 불가능하다고 생각하지 않았습니다.

- 질문 : 굳이 레포를 나눠야하나? 우리는 우형과 같은 트래픽을 받는게 아니지않나?
- 대답 : 우형처럼 15000 TPS를 기록하면서 아키텍처가 생존의 문제인 상황은 아니지만, 더 나은 아키텍처를 설계하는것은 중요하다고 생각합니다. 상품과 옵션 도메인은 비지니스의 확장이 지속적으로 있을텐데 백오피스 서비스와 여행자 서비스를 물리적으로 분리하는것은 나중에 성능 튜닝도 가능하게하는것 뿐만 아니라 유지보수를 용이하게 만들 수 있다고 생각합니다.

- 질문 : 여행자 페이지와 백오피스 페이지를 기준으로 서버를 나눈다면 화면에 따라 아키텍처가 결정되는것으로도 보이는데 맞을지?
- 대답 : 비지니스에 따라 나눈것인데, 화면도 다르다 보니깐 그렇게 생각할 수 있어보이네요. 중요한것은 화면에 따라 나누는게 아니라 비지니스가 다르니깐 나눈다입니다. 상품 상세와 상품 리스팅은 다른 화면이지만 같은 여행자 서버에 존재합니다. 

- 질문 : 공통 코드 중복 비용
- 대답 : 가장 저도 염려하는 부분인데요. 저희가 데이터가 비슷함에도 Create 서비스의 DTO와 Update 서비스의 DTO를 분리해서 가져가는것처럼, 이 부분도 중복된 부분이 일부 있지만 안고갈 수 있지않을까 생각해봤습니다. 또한 도메인 특성상 시간이 지날수록 디르게 변화할것이라고 생각합니다.

- 질문 : 백오피스에서도 조회기능이 있는데 커맨드 서비스라고 볼 수 있나?
- 대답 : 우형 래퍼런스의 영상에서도 가게업주(커맨드)이며 고객(쿼리)로 소개하고있는데요. 커맨드 서비스라고해서 조회가 없는것은 아닙니다. 가게업주가 자신이 등록한 광고를 당연히 볼 수 있어야겠죠.
  ...

하지만 이러한 과정속에서, 제가 발전하고 있다는걸 스스로 느낄 수 있었는데요. 자유롭게 의견을 펼칠 수 있는 문화와 분위기는 이래서 중요하구나 라고 생각했습니다. 

물론 자유롭게 의견을 펼치기 위해서는 동료들의 신뢰를 미리 얻어야한다고 생각합니다. 최범균님의 "육각형 개발자" 책을보면 어느정도는 묵묵히 주어진일을 수행하면서 신뢰를 쌓으라는 이야기가 있습니다. 

최근에 들었던 세미나중 하나가 5년차와 10년차 개발자의 차이가 무엇일까? 이었는데요. 우아콘에서도 연설하신 강사님의 말씀으로는 "바로 문화를 만들어갈 수 있냐 없냐" 였습니다. 
